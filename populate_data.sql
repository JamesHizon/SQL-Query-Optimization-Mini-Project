-- SQL Tuning Assignment:

-- Once the script have been executed successfully, open each SQL (linked) script and optimize
-- it if necessary to answer the questions.

-- Problems:
-- 1. List the name of the student w/ id equal to v1 (id).
-- 2. List the names of students w/ id in the range of v2 (id) to v3 (inclusive).
...
-- Approach:
	-- 1. Use "EXPLAIN ANALYZE" to observe statistics in regards to query.
    -- 2. Based on time shown in statistics, we want to make changes to our query.
    -- 3. When saving file, including the time it took to get all rows (in ms) prior to changes and
    -- w/ changes.
...
CREATE TABLE Course (
	...
	deptId VARCHAR(255),
	crsName VARCHAR(255),
    CONSTRAINT PRIMARY KEY(crsCode, deptId, crsName)
);
...
CREATE TABLE Transcript (
	...
	semester VARCHAR(255),
	grade VARCHAR(255),
    CONSTRAINT PRIMARY KEY(studId, crsCode) -- These two values are used to uniquely identify each row.
    -- ,
...
-- Missing index for constraint 'transcript_ibfk_2' in the referenced table 'Course'.

-- Error Code: 1822. Failed to add the foreign key constraint. Missing index for constraint 'FK_crsCode' in the referenced table 'Course'


-- Use "ALTER TABLE" to add foreign key constraints after creating composite primary key.
